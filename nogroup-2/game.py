...
def Game(screen):
    ...
    while game_running:
        ...
        if state == game_running:            
            ...
            lifestr = str(player.life)
            scorestr = str(score)
        
            # Test section for timer, lives, score and kill counter 
            myfont = pygame.font.SysFont('Hoefler', 50)
            lives = myfont.render("Lives: " + lifestr, False, (153, 0, 153))
            score_total = myfont.render("Score: " + scorestr, False, (153, 0, 153))
            
            
          
            screen.blit(lives,(0,0))
            screen.blit(score_total,(0,70))
